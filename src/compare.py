#!/usr/bin/env python3
"""
Creates side-by-side per-sample before/after class-timeline plots using a fixed threshold
(from zero_fp_threshold.json). Also produces a CSV summary with counts.
...
def to_classes(scores, thr):
    # scores: list of floats -> classes 0/1
...
def plot_pair(before_cls, after_cls, sid, out_path, thr):
    # Make a 2-panel figure: left=before, right=after
    ...
    # highlight windows that changed 1 -> 0 (successful evasion)
    ...
    for i in range(min_len):
        if before_cls[i] == 1 and after_cls[i] == 0:
            changed.append(i)
    if changed:
        # plot vertical markers on both subplots
        for ax in axes:
            for idx in changed:
...
def main():
    ...
    # Build set of sample ids present in either file
    ...
    for sid in sample_ids:
        ...
        if binfo is None or ainfo is None:
            # skip if sample missing in one of the files
            continue
